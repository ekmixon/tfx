...
class Evaluator(base_beam_component.BaseBeamComponent):
  ...
  def __init__(
    ...
    Args:
      ...
      example_splits: Names of splits on which the metrics are computed. Default
        behavior (when example_splits is set to None or Empty) is using the
        'eval' split.
      ...
      module_file: A path to python module file containing UDFs for Evaluator
        ...
        level. def custom_eval_shared_model( eval_saved_model_path, model_name,
        eval_config, **kwargs,
          ) -> tfma.EvalSharedModel: def custom_extractors( eval_shared_model,
